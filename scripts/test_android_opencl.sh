...
run_remote() {
  adb "${adb_args[@]}" shell \
    "cd '${remote_dir}' && export LD_LIBRARY_PATH='${remote_dir}/lib:/vendor/lib64:/system/vendor/lib64' && $*" 2>&1
}

restore_backend() {
  adb "${adb_args[@]}" shell \
    "if [ -f '${remote_dir}/lib/libggml-opencl.so.disabled' ]; then mv '${remote_dir}/lib/libggml-opencl.so.disabled' '${remote_dir}/lib/libggml-opencl.so'; fi" \
    >/dev/null 2>&1 || true
}
...
printf '%s\n' "${backend_output}"
grep -F 'accelerator=OPENCL' <<<"${backend_output}"
grep -F 'selected_device=GPUOpenCL' <<<"${backend_output}"
...
printf '%s\n' "${build_output}"
grep -F '[LlamaCppEmbedding] Loaded OPENCL model on GPUOpenCL' <<<"${build_output}"
grep -F 'ggml_opencl: device:' <<<"${build_output}"
if grep -Eiq 'will fall back|fall back to CPU|fallback to CPU' <<<"${build_output}"; then
  echo "OpenCL build reported an accelerator fallback" >&2
  exit 1
fi
...
printf '%s\n' "${retrieval_output}"
grep -F '[LlamaCppReranker] Loaded OPENCL model on GPUOpenCL' <<<"${retrieval_output}"
...
if grep -Eiq 'will fall back|fall back to CPU|fallback to CPU' <<<"${retrieval_output}"; then
  echo "OpenCL retrieval/reranking reported an accelerator fallback" >&2
  exit 1
fi
...
grep -F '[LlamaCppModel] Loaded OPENCL model on GPUOpenCL' <<<"${generation_output}"
if grep -Eiq 'will fall back|fall back to CPU|fallback to CPU' <<<"${generation_output}"; then
  echo "OpenCL generation reported an accelerator fallback" >&2
  exit 1
fi
...
if [[ "${NATIVERAG_RUN_NEGATIVE_TEST:-0}" == "1" ]]; then
  ...
  adb "${adb_args[@]}" shell \
    "mv '${remote_dir}/lib/libggml-opencl.so' '${remote_dir}/lib/libggml-opencl.so.disabled'"
  if run_remote "./mobile_rag_cli --backend-info"; then
    echo "OpenCL negative test failed: program ran without libggml-opencl.so" >&2
    exit 1
  fi
  restore_backend
  trap - EXIT
...
echo "Android OpenCL Local RAG smoke test passed"
