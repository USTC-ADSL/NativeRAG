...
void testLoadDataset() {
    std::cout << "\n=== Test 1: Check Pre-generated Embeddings Existence ===" << std::endl;
    ...
    assert(success);
    std::cout << "✓ Found vectors.bin and metadata.txt under dataset/data/qwen3_embeddings" << std::endl;
}

// 测试2：使用预生成向量（不再从数据集生成）
void testGenerateVectorsFromDataset() {
    std::cout << "\n=== Test 2: Use Pre-generated Embeddings ===" << std::endl;
    std::cout << "✓ Using dataset/data/qwen3_embeddings as input" << std::endl;
}
...
void testLoadVectors() {
    ...
    std::string vectors_file = std::string(PROJECT_ROOT_DIR) + "/dataset/data/qwen3_embeddings/vectors.bin";
...
void testMetadata() {
    ...
    std::string metadata_file = std::string(PROJECT_ROOT_DIR) + "/dataset/data/qwen3_embeddings/metadata.txt";
